...
\begin{frame}[standout]
The theory matches TA, and matches key features of randomly initialized networks, even in the deeper case.
\end{frame}
...
\begin{frame}[standout]
Generalization bounds are loose because they completely ignore structure in the data.\\[1em]
This is partly because they're based on binary classification, and modern tasks are much richer.
\end{frame}
...
\centering
\only<1>{
\includegraphics[height=0.8\textheight]{figures/other_peoples/zhang_2017_slowdown.png}
}
\only<2->{
...
\only<4->{
\includegraphics[width=0.8\textwidth]{figures/other_peoples/zhang_2017_slowdown.png}
}
\end{subfigure}%%
\begin{subfigure}[t]{0.5\textwidth}
\only<3->{%%
...
\node at (0, 3) (output) {\only<2->{\LARGE {Play at C11}}};
...
\uncover<2->{
It's more beneficial when the target task is low SNR, and the auxiliary tasks are well aligned. \\[1em]}
\uncover<3->{
This is precisely the regime of data-efficient human learning.
}
\end{frame}
...
\item Generalization dynamics:
    ...
    \item But the strongest modes are least distorted and first to be learned.
...
\item Therefore:
    \begin{itemize}
    \item Generalization bounds based only on capacity for memorization are na\:{i}ve, memorization is slower.
