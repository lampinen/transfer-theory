...
noise_vars = [500, 100, 10, 1] # scaled independent gaussian per output
num_runs = 1 
learning_rate = 0.001
num_epochs = 5000
num_hidden = 10
...
###
var_scale_init = tf.contrib.layers.variance_scaling_initializer(factor=epsilon, mode='FAN_AVG')
    ...
    for noise_var in noise_vars:
        for output_size in output_sizes:
            ...
            if input_type == "one_hot":
                x_data, y_data, noisy_y_data, input_modes = datasets.noisy_SVD_dataset(num_examples, output_size, noise_var=scaled_noise_var, singular_value_multiplier=singular_value_multiplier) 
            else:
                x_data, y_data, noisy_y_data, input_modes = datasets.noisy_SVD_dataset_different_inputs(num_examples, output_size, noise_var=scaled_noise_var, input_type="orthogonal", singular_value_multiplier=singular_value_multiplier) 
            ...
            np.savetxt(filename_prefix + "noise_var_%.2f_output_size_%i_run_%i_y_data.csv", y_data, delimiter=",")
            np.savetxt(filename_prefix + "noise_var_%.2f_output_size_%i_run_%i_noisy_y_data.csv", noisy_y_data, delimiter=",")
            if input_type != "one_hot":
                np.savetxt(filename_prefix + "noise_var_%.2f_output_size_%i_run_%i_x_data.csv", x_data, delimiter=",")
