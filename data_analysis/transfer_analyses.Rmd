...
for (noise_var in noise_vars) {
  for (output_size in output_sizes) {
    for (run in 0:(nruns-1)) { 
      this_d = read.csv(sprintf("../single_generalization_full_results_notf_randi/noise_var_%.2f_output_size_%i_run_%i.csv", noise_var, output_size, run), header=T)
  ...
  this_d = read.csv(sprintf("../single_generalization_full_results/noise_var_%.2f_theory_track.csv", noise_var), header=T)
...
```{r}
ggsave("../plots/theory_and_empirical_full_rank.png")
```
