...
```{r}
p = ggplot(avg_d %>% 
         filter(epoch %% 100 == 0,  num_examples >= 100, type=="empirical", num_examples == 200, input_type=="orthogonal"), 
  ...
  geom_point(size=2) +
  geom_line(data=avg_d %>% filter( num_examples == 200, type=="theory"), aes(linetype=factor("Theory")), size=1.5) +
