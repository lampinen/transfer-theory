...
```{r}
min_d = d %>%
  group_by(svm, p, depth, aligned, noise_var, type, teacher_rank, student_rank, run) %>%
  summarize(min_Eg = min(log_loss), min_t = which.min(log_loss)) %>%
  ungroup() %>%
  group_by(svm, p, depth, aligned, noise_var, teacher_rank, student_rank, type) %>%
  summarize(avg_min_Eg = mean(min_Eg), sd_min_Eg = sd(min_Eg),
            avg_min_t = mean(min_t), sd_min_t = sd(min_t),
            se_min_Eg = sd_min_Eg/sqrt(n()), ci_Eg_up= avg_min_Eg + 1.96*se_min_Eg, ci_Eg_lo= avg_min_Eg - 1.96*se_min_Eg,
            se_min_t = sd_min_t/sqrt(n()), ci_t_up= avg_min_t + 1.96*se_min_t, ci_t_lo= avg_min_t - 1.96*se_min_t) %>%
...
```{r}
make_changing_p_plots = function(avg_d, wide_min_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
  # train 
  p = ggplot(avg_d %>% filter(type=="Theory", svm==3.), aes(x=epoch/(1000), y=log_tr_loss, color=as.numeric(levels(p)[p])/100, group=interaction(new_type, svm, p, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), svm==3., epoch %% 1000 == 0), aes(shape=aligned), size=3) +
    scale_x_continuous(breaks=x_breaks) +
    scale_color_gradient(low="#008800", high="#FFD700", limits=c(0, 2), breaks=c(0., 1., 2.)) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{train}$")) +
    guides(color=F, linetype=F, shape=F)+
    annotation_custom(textGrob(label = lets[1], hjust = 2.5, gp = gpar(cex = 2),
  ...
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/(1000), y=log_loss, color=as.numeric(levels(p)[p])/100, group=interaction(new_type, svm, p, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 1000 == 0), aes(shape=aligned), size=3) +
    scale_color_gradient(low="#008800", high="#FFD700", limits=c(0, 2), breaks=c(0., 1., 2.)) +
    scale_x_continuous(breaks=x_breaks) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{test}$")) +
...
```{r}
make_changing_p_plots(avg_d, wide_min_d,
