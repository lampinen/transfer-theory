...
```{r}
make_randomizing_fig_plots = function(avg_d, spectra_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
    # spectra
    p = ggplot(spectra_d, aes(x=singular_value, color=randomized)) +
    geom_histogram(position="dodge") +
    scale_color_brewer(palette="Dark2") +
    labs(x=TeX("$s$"),
         y=TeX("Density")) +
    annotation_custom(textGrob(label = lets[1], hjust = 3, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) +
    guides(color=F)
  ...
  # train 
  p = ggplot(avg_d %>% filter(epoch %% 50 == 0), aes(x=epoch/1000, y=tr_loss, color=randomized, group=interaction(new_type, svm))) +
    geom_point(size=2) +
    scale_x_continuous(breaks=x_breaks) +
    scale_color_brewer(palette="Dark2") +
    labs(x=TeX("$t/\\tau$"),
...
```{r}
# make_randomizing_fig_plots(avg_d %>% filter(randomized=="True"),
#                            file_prefix="../plots/paper/randomized_fig_",
#                            lets=c("C"))
```
