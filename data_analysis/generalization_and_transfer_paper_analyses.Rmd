...
make_fig3_plots = function(avg_d, wide_min_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
  ...
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/1000, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=2) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 200 == 0), aes(shape=aligned), size=2) +
...
make_randomizing_fig_plots = function(avg_d, spectra_d, file_prefix, lets, x_breaks=c(0., 2.)) {
    ...
    labs(x=TeX("$s$"),
         y=TeX("Count")) +
    annotation_custom(textGrob(label = lets[1], hjust = 1, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) +
    facet_grid(randomized ~ .) +
    guides(fill=guide_legend(title="Randomized"), color=guide_legend(title="Theory")) +
  ...
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="False"), aes(xintercept=inflated1, color="Signal values"), linetype="dashed", size=0.66)
  s=2.
  this_s = s * this_svm
  inflated2 = sqrt((1+this_s^2)*(0.5+this_s^2))/this_s
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="False"), aes(xintercept=inflated2, color="Signal values"), linetype="dashed", size=0.66)
  s=3.
  this_s = s * this_svm
  inflated3 = sqrt((1+this_s^2)*(0.5+this_s^2))/this_s
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="False"), aes(xintercept=inflated3, color="Signal values"), linetype="dashed", size=0.66)
  
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="False"), aes(xintercept=1-sqrt(0.5), color="Noise endpoints"), linetype="dashed", size=0.66)
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="False"), aes(xintercept=1+sqrt(0.5), color="Noise endpoints"), linetype="dashed", size=0.66)
  ...
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="True"), aes(xintercept=sigma_r*(1-sqrt(0.5)), color="Noise endpoints"), linetype="dashed", size=0.66)
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="True"), aes(xintercept=sigma_r*(1+sqrt(0.5)), color="Noise endpoints"), linetype="dashed", size=0.66)
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  ...
  p = ggplot(avg_d %>% filter(epoch %% 10 == 0), aes(x=epoch/1000, y=tr_loss, color=randomized, group=interaction(randomized, svm))) +
    ...
    guides(color=guide_legend(title="Randomized"))
