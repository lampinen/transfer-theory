...
```{r}
nruns = 10
ps = c(20, 100, 160, 200)#, 40, 60, 80, 120, 140, 160, 180)
noise_vars = c(1.)
output_sizes= c(100)
svms = c(2., 3.)
alignments = c("True")
    ...
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (aligned in alignments) {
            for (run in 0:(nruns-1)) { 
              filename = sprintf("../changing_p_results/noise_var_%.2f_output_size_%i_input_size_100_num_examples_%i_input_type_orthogonal_svm_%f_aligned_%s_run_%i.csv", noise_var, output_size, p, svm, aligned, run)
              if (!file.exists(filename)) {
                next
              }
              this_d = read.csv(filename, header=T)
              ...
              this_d = this_d %>% 
                mutate(noise_var=noise_var, output_size=output_size, svm=svm, aligned=aligned=="True", run=run, p=p, depth=1, teacher_rank=1, student_rank="Full rank", type="Empirical")
              d = bind_rows(d, this_d)
            }
          }
          ...
          if(file.exists(theory_filename)) { # min_d only theory is omitted
            ...
            this_d = this_d %>% 
              rename(loss=generalization_error) %>%
              mutate(noise_var=noise_var, output_size=NA,  svm=svm, aligned=aligned=="True", p=p, depth=1, teacher_rank=1, run=NA, student_rank="Full rank", type="Theory")
            d = bind_rows(d, this_d)
          }
        }
      }
...
```{r}
min_d = d %>%
  group_by(svm, rt_D_x_snr, p, depth, aligned, noise_var, type, teacher_rank, student_rank, run) %>%
  summarize(min_Eg = min(log_loss), min_t = which.min(log_loss)) %>%
  ungroup() %>%
  group_by(svm, p, rt_D_x_snr, depth, aligned, noise_var, teacher_rank, student_rank, type) %>%
  summarize(avg_min_Eg = mean(min_Eg), sd_min_Eg = sd(min_Eg),
            avg_min_t = mean(min_t), sd_min_t = sd(min_t),
            se_min_Eg = sd_min_Eg/sqrt(n()), ci_Eg_up= avg_min_Eg + 1.96*se_min_Eg, ci_Eg_lo= avg_min_Eg - 1.96*se_min_Eg,
            se_min_t = sd_min_t/sqrt(n()), ci_t_up= avg_min_t + 1.96*se_min_t, ci_t_lo= avg_min_t - 1.96*se_min_t) %>%
...
```{r}
make_changing_p_plots = function(avg_d, min_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
  # train 
  ...
  p = ggplot(data=min_d %>% filter(type=="Empirical"), aes(x=rt_D_x_snr, y=min_Eg, color=as.numeric(levels(svm)[svm]))) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 10), breaks=c(0.84, 10), labels=c( parse(text="sqrt(1/2, 4)"), "10      ")) +
    labs(x=TeX("\\sqrt{P/N_1}\cdot \\text{SNR}"),
         y=TeX("$\\min \\, \\ln \\,\\epsilon_{test}$ (TA)")) +
    geom_line(data=min_d %>% filter(type=="Theory"), size=1.5) +
...
```{r}
make_changing_p_plots(avg_d, min_d,
