    ...
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (aligned in alignments) {
            for (run in 0:(nruns-1)) { 
              filename = sprintf("../sg_nl_results_orth/depth_%i_n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_run_%i_.csv", depth, n2b, noise_var, output_size, svm, run)
              if (!file.exists(filename)) {
                next
              }
              this_d = read.csv(filename, header=T)
...
```{r}
make_nonlinear_fig_plots = function(avg_d, file_prefix, lets, x_breaks=c(0., 5., 10.), legend=F) {
  # train 
  p = ggplot(avg_d %>% filter(epoch %% 50 == 0), aes(x=epoch/1000, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm))) +
    geom_line() +
    geom_point(size=2) +
    scale_x_continuous(breaks=x_breaks) +
  ...
  # test
  p = ggplot(avg_d %>% filter(epoch %% 50 == 0), aes(x=epoch/1000, y=log_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm))) +
    geom_line() +
    geom_point(size=2) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 30), breaks=c(1., 30)) +
    scale_x_continuous(breaks=x_breaks) +
