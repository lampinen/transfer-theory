...
p = ggplot(data=d, aes(x=sbar)) +
  ...
  geom_line(aes(y=pred_MP_S_hat, color="#7570B3"), size=1.1, linetype=2) +
  scale_fill_identity(name="Empirical", guide="none", labels=c("Empirical")) +
  scale_color_identity(name="Theoretical", guide="none", breaks=c("#D95F02", "#7570B3"), labels=c("Signal", "Noise")) +
...
```{r}
ggsave("../plots/paper/fig_2a.png", plot=gt, width=2.75, height=3)
```
...
p = ggplot(d %>% filter(sbar <= 5.), aes(x=sbar, y=S_hat, color=factor(A), group=A)) +
  ...
  annotation_custom(textGrob(label = "C", hjust = 2, gp = gpar(cex = 2),
      x=unit(0, "npc"), y=unit(0.95, "npc")),
      )+ 
...
```{r}
ggplot(data=avg_d, aes(x=epoch, y= mean_loss, color=num_inputs)) + 
  geom_point() +
...
avg_d = d %>%
  ...
  mutate(new_type = ifelse(type == "Theory", "Theory",
                            ifelse(aligned == "TRUE", "Empirical aligned", "Empirical random"))) %>%
  mutate(new_type = factor(new_type, levels=c("Theory", "Empirical aligned", "Empirical random")),
         aligned=factor(aligned, levels=c("TRUE", "FALSE"), labels=c("Random", "TA")))
```
...
```{r}
ggsave("../plots/paper/fig_3a.png", plot=gt, width=3.8, height=3.6)
```
...
```{r}
ggsave("../plots/paper/fig_3b.png", plot=gt, width=5, height=3.6)
```
...
```{r}
p = ggplot(data=dapprox, aes(x=singular_value_multiplier, y=log(approx_opt_test_error), color=factor(A))) +
  scale_color_manual(values=c("#A1D99B", "#41AB5D", "#238B45", "#00441B"), breaks=c(0.25,0.5,0.75,1.)) +
  labs(x = "SNR",
       y=TeX("$\\min \\, \\ln \\,\\epsilon_{test}$ (TA)")) +
  geom_line(aes(linetype=factor("Exact\ntheory")), size=2) +
  geom_point(data=wide_min_d %>% mutate(A=factor(0.5)), aes(shape=factor("Approx.\ntheory"), x=as.numeric(levels(svm)[svm]), y=Aligned_avg_min_Eg, color=A), size=3) + 
