...
noise_vars = c(1.)
output_sizes= c(50)
svms = c(0.84, 2., 3., 6., 10., 20., 100.)
d = data.frame()
for (noise_var in noise_vars) {
  for (output_size in output_sizes) {
    for (svm in svms) {
      for (run in 0:(nruns-1)) { 
        this_d = read.csv(sprintf("../paper_nonlinear_single_generalization_results/noise_var_%.2f_output_size_%i_svm_%f_run_%i_.csv",noise_var, output_size, svm, run), header=T)
...
```{r}
p = ggplot(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 200 == 0, as.numeric(levels(svm)[svm]) <= 10), aes(x=epoch/1000, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
  geom_point(aes(shape=aligned), size=2) +
 geom_line( size=0.5) +
  geom_errorbar(aes(ymin=log_tr_loss_95_low, ymax=log_tr_loss_95_high)) +
  scale_x_continuous(breaks=c(0., 5., 10.)) +
  scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 10), breaks=c(0.84, 100), labels=c( parse(text="sqrt(1/2, 4)"), "100   ")) +
  labs(x=TeX("$t/\\tau$"),
       y=TeX("$\\ln \\, \\epsilon_{train}$")) +
  guides(color=F, linetype=F, shape=F)+
  annotation_custom(textGrob(label = "A", hjust = 3, gp = gpar(cex = 2),
...
```{r}
ggsave("../plots/paper/supp_nonlinear_gen_A.png", plot=gt, width=3, height=3)
```


```{r}
p = ggplot(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 200 == 0), aes(x=epoch/1000, y=log_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
 geom_line( size=0.5) +
  geom_point(aes(shape=aligned), size=2) +
  geom_errorbar(aes(ymin=log_loss_95_low, ymax=log_loss_95_high)) +
#  geom_ribbon(aes(ymin=log_loss_95_low, ymax=log_loss_95_high, fill=features), alpha=0.5) +
  scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 100), breaks=c(0.84, 100), labels=c( parse(text="sqrt(1/2, 4)"), "100     ")) +
  scale_x_continuous(breaks=c(0., 5., 10.)) +
  labs(x=TeX("$t/\\tau$"),
       y=TeX("$\\ln \\, \\epsilon_{test}$")) +
  guides(color=guide_colorbar(title="SNR", order=3), linetype=guide_legend(title="", order=1), shape=guide_legend(title="Empirical", order=2))+
...
```{r}
ggsave("../plots/paper/supp_nonlinear_gen_B.png", plot=gt, width=4, height=3)
```
...
```{r}
p = ggplot(avg_d %>% filter(type=="Theory", svm %in% c(2., 3.)), aes(x=epoch/1000, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
 geom_line(aes(linetype=factor("Theory")), size=1.5) +
  geom_point(data=avg_d %>% filter(grepl("Empirical", type), svm %in% c(2., 3.), aligned=="TA", epoch %% 200 == 0), aes(shape=aligned), size=2) +
#  geom_ribbon(aes(ymin=log_loss_95_low, ymax=log_loss_95_high, fill=features), alpha=0.5) +
  scale_x_continuous(breaks=c(0., 75., 150.)) +
...
```{r}
p = ggplot(avg_d %>% filter(type=="Theory", svm %in% c(2., 3.), epoch %% 1000 == 1), aes(x=epoch/1000, y=log_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
 geom_line(aes(linetype=factor("Theory")), size=1.5) +
  geom_point(data=avg_d %>% filter(grepl("Empirical", type), svm %in% c(2., 3.), aligned=="TA", epoch %% 200 == 0), aes(shape=aligned), size=2) +
#  geom_ribbon(aes(ymin=log_loss_95_low, ymax=log_loss_95_high, fill=features), alpha=0.5) +
  scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 4), breaks=c(0.84, 4), labels=c( parse(text="sqrt(1/2, 4)"), "4      ")) +
