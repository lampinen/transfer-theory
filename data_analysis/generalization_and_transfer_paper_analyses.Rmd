...
```
```{r}
ggsave("../plots/paper/fig_2a.eps", plot=gt, width=6, height=3)
```
...
```{r}
make_fig3_plots = function(avg_d, wide_min_d, file_prefix, lets, x_breaks=c(0., 5., 10.), image_extension=".png") {
  # train 
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/1000, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[1], image_extension, sep=""), plot=gt, width=3, height=3)
...
#    guides(color=guide_colorbar(title="SNR", order=3), linetype=guide_legend(title="", order=1), shape=guide_legend(title="Empirical", order=2))+
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[2], image_extension, sep=""), plot=gt, width=4.25, height=3)
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[3], image_extension, sep=""), plot=gt, width=3, height=3)
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[4], image_extension, sep=""), plot=gt, width=3, height=3)
}
```
