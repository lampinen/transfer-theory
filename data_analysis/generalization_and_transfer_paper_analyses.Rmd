...
```{r}
ggplot(da, aes(x=t, y=s_of_t, color=s_hat, group=s_hat)) +
  ...
  scale_x_continuous(breaks =c(0, 1.036, 6), labels=c("0", parse(text="t[4]"), "10")) +
  scale_y_continuous(breaks =c(0, 4, 10)) +
  coord_cartesian(xlim=c(0,6), ylim=c(0,10)) +
...
```{r}
ggsave("../plots/paper/fig_1a.png", width=4, height=3)
```

```{r}
ggplot(db %>% filter(t <= 4), aes(x=s_hat, y=s_of_t/s_hat, color=t, group=t)) +
#  annotate("segment", y = 4, yend=4, x=-1, xend=6, alpha=0.5, linetype=2, size=1, color="#AD00AF") +
#  annotate("segment", y = -1, yend=11, x=1.036, xend=1.036, alpha=0.5, linetype=2, size=1, color="#AD00AF") +
  ...
  geom_line(size=2) +
  scale_color_gradient(low="#AA00CC", high="#EEEE00", limits=c(0.5, 4), breaks=c(0.5, 1:4))  +
...
```{r}
ggsave("../plots/paper/fig_1b.png", width=4, height=3)
```
