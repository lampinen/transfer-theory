...
#    guides(color=guide_colorbar(title="SNR", order=3), linetype=guide_legend(title="", order=1), shape=guide_legend(title="Empirical", order=2))+
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[2], ".png", sep=""), plot=gt, width=3, height=3)
  ...
  p = ggplot(data=wide_min_d, aes(x=Aligned_avg_min_t/1000, y=Random_avg_min_t/1000, color=as.integer(svm))) +
    ...
    theme(legend.margin = margin(0.,0,-0.6,0, unit="cm"))
  
  
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[4], ".png", sep=""), plot=gt, width=4.25, height=3)
}
```
          ...
          for (aligned in alignments) {
            for (input_type in input_types) {
              if (input_type == "orthogonal") {
                ...
                this_d = "changing_p_results_gaussian"
              }
              for (run in 0:(nruns-1)) { 
...
```{r}
avg_d = d %>%
  filter(svm == 3., p %in% c(20, 100, 200)) %>%
...
make_changing_p_plots = function(avg_d, min_d, wide_min_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
  # train 
  p = ggplot(avg_d %>% filter(type=="Theory", svm==3.), aes(x=epoch/(1000), y=log_tr_loss, color=as.numeric(levels(p)[p])/100, group=interaction(new_type, svm, p, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
  ...
  # test
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/(1000), y=log_loss, color=as.numeric(levels(p)[p])/100, group=interaction(new_type, svm, p, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
  ...
  p = ggplot(data=min_d %>% filter(type=="Empirical"), aes(x=rt_D_x_snr, y=avg_min_Eg, color=as.numeric(levels(svm)[svm]))) +
  ...
  p = ggplot(data=wide_min_d, aes(x=orthogonal_avg_min_Eg, y=gaussian_avg_min_Eg, color=as.numeric(levels(p)[p])/100, shape=svm)) +
    ...
    geom_point(size=2) +
    guides(color=guide_colorbar(title=TeX("$P/N_1$")), shape=guide_legend(title="SNR"))  +
    annotation_custom(textGrob(label = lets[4], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.97, "npc"))) +
    theme(legend.margin = margin(-0.2,0,-0.2,0, unit="cm"))
