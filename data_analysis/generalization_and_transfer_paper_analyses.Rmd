...
p = ggplot(d %>% filter(sbar <= 5.), aes(x=sbar, y=C, color=factor(A), group=A)) +
  geom_line(size=2) +
  geom_point(aes(y=C_emp_mean)) +
  geom_errorbar(aes(ymin = C_emp_mean - 1.96*C_emp_sem, ymax=C_emp_mean + 1.96 * C_emp_se)) +
  scale_color_manual(values=c("#A1D99B", "#41AB5D", "#238B45", "#00441B"), breaks=c(0.25,0.5,0.75,1.)) +
...
```{r}
ggsave("../plots/paper/fig_2d.png", plot=gt, width=4, height=3)
```
