...
p = ggplot(data=d, aes(x=sbar)) +
  ...
  labs(x=parse(text="hat(s)"), y="Count")+
    ...
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (aligned in alignments) {
            for (run in 0:(nruns-1)) { 
              filename = sprintf("../sg_%il_results/n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_aligned_%s_run_%i.csv", depth, n2b, noise_var, output_size, svm, aligned, run)
              ...
              this_d = this_d %>% 
                mutate(noise_var=noise_var, output_size=output_size, svm=svm, aligned=aligned=="True", run=run, depth=depth, teacher_rank=n2b, student_rank="Full rank", type="Empirical")
              d = bind_rows(d, this_d)
            }
          }
          theory_filename = sprintf("../sg_%il_results/n2b_%i_noise_var_%.2f_p_100_svm_%f_theory_track.csv", depth, n2b, noise_var, svm)
          if(file.exists(theory_filename)) { # min_d only theory is omitted
            this_d = read.csv(theory_filename, header=T)
          
            this_d = this_d %>% 
              filter(epoch <=10000 | depth==3) %>% # generated too long of tracks
              rename(loss=generalization_error) %>%
              mutate(noise_var=noise_var, output_size=NA,  svm=svm, aligned=aligned=="True",  depth=depth, teacher_rank=n2b, run=NA, student_rank="Full rank", type="Theory")
            d = bind_rows(d, this_d)
          }
        }
      }
...
```{r}
min_d = d %>%
  group_by(svm, depth, aligned, noise_var, type, teacher_rank, student_rank, run) %>%
  summarize(min_Eg = min(log_loss), min_t = which.min(log_loss)) %>%
  ungroup() %>%
  group_by(svm, depth, aligned, noise_var, teacher_rank, student_rank, type) %>%
  summarize(avg_min_Eg = mean(min_Eg), sd_min_Eg = sd(min_Eg),
            avg_min_t = mean(min_t), sd_min_t = sd(min_t),
            se_min_Eg = sd_min_Eg/sqrt(n()), ci_Eg_up= avg_min_Eg + 1.96*se_min_Eg, ci_Eg_lo= avg_min_Eg - 1.96*se_min_Eg,
            se_min_t = sd_min_t/sqrt(n()), ci_t_up= avg_min_t + 1.96*se_min_t, ci_t_lo= avg_min_t - 1.96*se_min_t) %>%
...
```{r}
avg_d = d %>%
  group_by(depth, noise_var, output_size, svm, aligned, epoch, teacher_rank,student_rank, type) %>%
  summarize(loss=mean(loss), log_tr_loss_sd=sd(log_train_loss), log_tr_loss=mean(log_train_loss), log_loss_sd=sd(log_loss), log_loss=mean(log_loss)) %>%
  mutate(log_loss_95_low=log_loss-1.96*log_loss_sd/sqrt(n()), log_loss_95_high=log_loss+1.96*log_loss_sd/sqrt(n()),
         log_tr_loss_95_low=log_tr_loss-1.96*log_tr_loss_sd/sqrt(n()), log_tr_loss_95_high=log_tr_loss+1.96*log_tr_loss_sd/sqrt(n())) %>%
  ungroup() %>%
...
```{r}
make_fig3_plots = function(avg_d, wide_min_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
  # train 
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/1000, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 250 == 0), aes(shape=aligned), size=3) +
    scale_x_continuous(breaks=x_breaks) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 10), breaks=c(0.84, 10), labels=c( parse(text="sqrt(1/2, 4)"), "10      ")) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{train}$")) +
    guides(color=F, linetype=F, shape=F)+
    annotation_custom(textGrob(label = lets[1], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) 
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[1], ".png", sep=""), plot=gt, width=3, height=3)
  
  # test
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/1000, y=log_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 250 == 0), aes(shape=aligned), size=3) +
  #  geom_ribbon(aes(ymin=log_loss_95_low, ymax=log_loss_95_high, fill=features), alpha=0.5) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 10), breaks=c(0.84, 10), labels=c( parse(text="sqrt(1/2, 4)"), "10      ")) +
    ...
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{test}$")) +
    guides(color=guide_colorbar(title="SNR", order=1, barheight=unit(2.5, "cm")),
           linetype=guide_legend(title="", order=2, override.aes=list(alpha=c(1), linetype=c("solid"))),
           shape=guide_legend(title="Empirical", order=3)) +
#    guides(color=guide_colorbar(title="SNR", order=3), linetype=guide_legend(title="", order=1), shape=guide_legend(title="Empirical", order=2))+
    annotation_custom(textGrob(label = lets[2], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) + 
    theme(legend.margin = margin(0.1,0,-0.5,0, unit="cm"))
  
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[2], ".png", sep=""), plot=gt, width=4.25, height=3)
  ...
  p = ggplot(data=wide_min_d, aes(x=Aligned_avg_min_Eg, y=Random_avg_min_Eg, color=as.numeric(svm))) +
    ...
    geom_point(size=3) +
    guides(color=F, shape=F)  +
    annotation_custom(textGrob(label = lets[3], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.97, "npc")),
        ) 
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[3], ".png", sep=""), plot=gt, width=3, height=3)
  ...
  p = ggplot(data=wide_min_d, aes(x=Aligned_avg_min_t/1000, y=Random_avg_min_t/1000, color=as.integer(svm))) +
    ...
    labs(x=TeX("$t_{opt}/\\tau$ (TA)"),
         y=TeX("$t_{opt}/\\tau$ (Rand.)")) +
    geom_point(aes(shape=factor("Empirical")), size=3) +
    geom_errorbar(aes(ymin=Random_ci_t_lo/1000, ymax=Random_ci_t_up/1000), width=0.01) +
    geom_errorbarh(aes(xmin=Aligned_ci_t_lo/1000, xmax=Aligned_ci_t_up/1000), height=0.01) +
    guides(color=F, linetype=F, shape=F)+
    annotation_custom(textGrob(label = lets[4], hjust = 3, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) + 
    theme(legend.margin = margin(-0.05,0,-0.5,0, unit="cm"))
  
  
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[4], ".png", sep=""), plot=gt, width=3, height=3)
}
```


```{r}
make_fig3_plots(avg_d %>% filter(teacher_rank==1,
                                 epoch <= 5001,
                                 svm %in% c(2., 4., 8.),
                                 depth==1),
                wide_min_d %>% filter(teacher_rank==1,
                                 depth==1),
                file_prefix="../plots/paper/fig3_redux_",
                lets=c("A", "B", "C", "D"),
                x_breaks=c(0., 5.))
```

```{r}
make_fig3_plots(avg_d %>% filter(teacher_rank==3,
                                 epoch <= 5001,
                                 svm %in% c(2., 4., 8.),
                                 depth==1),
                wide_min_d %>% filter(teacher_rank==3,
                                 depth==1),
                file_prefix="../plots/paper/fig3_redux_",
                lets=c("E", "F", "G", "H"),
                x_breaks=c(0., 5.))
...
make_fig3_plots(avg_d %>% filter(teacher_rank==1,
                                 epoch <= 120001,
                                 epoch %% 5000 == 0 | ((type == "Theory") & (epoch %% 1000 == 1)),
                                 svm %in% c(2., 4., 8.),
                                 depth==3,
                                 new_type != "Empirical random"),
                wide_min_d %>% filter(teacher_rank==1,
                                 depth==3),
                file_prefix="../plots/paper/deep_fig_redux_",
                lets=c("A", "B", "C", "D"),
                x_breaks=c(0., 60., 120.))
```
...
make_fig3_plots(avg_d %>% filter(teacher_rank==3,
                                 epoch <= 120001,
                                 epoch %% 5000 %in% c(0,1) | (type == "Theory" & epoch %% 1000 == 1),
                                 svm %in% c(2., 4., 8.),
                                 depth==3,
                                 new_type != "Empirical random"),
                wide_min_d %>% filter(teacher_rank==3,
...
```{r}
# unaligned test
p = ggplot(avg_d %>% filter(new_type == "Empirical random",
                            teacher_rank==3,
                            epoch %% 500 %in% c(0,1),
                            svm %in% c(2., 4., 8.),
                            depth==3), 
           aes(x=epoch/1000, y=log_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm, student_rank))) +
# geom_line(aes(linetype=factor("Theory")), size=1.5) +
  geom_point(shape=17, size=3) +
#  geom_ribbon(aes(ymin=log_loss_95_low, ymax=log_loss_95_high, fill=features), alpha=0.5) +
  scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 10), breaks=c(0.84, 10), labels=c( parse(text="sqrt(1/2, 4)"), "10      ")) +
  scale_x_continuous(limits=c(0, 120), breaks=c(0., 60., 120.)) +
  labs(x=TeX("$t/\\tau$"),
       y=TeX("$\\ln \\, \\epsilon_{test}$")) +
  guides(color=F, linetype=F, shape=F)+
  theme(legend.margin = margin(0.1,0,-0.5,0, unit="cm"))
...
gt$layout$clip[gt$layout$name == "panel"] <- "off"
grid.draw(gt)
ggsave(paste("../plots/paper/deep_fig_redux_", "F", "_unaligned.png", sep=""), plot=gt, width=3, height=3)
```

```{r}
# mode dips / suboptimality
p = ggplot(avg_d %>% filter(type == "Theory",
                            teacher_rank==1,
                            epoch < 5001,
                            svm %in% c(4.),
                            depth==1), 
...
gt$layout$clip[gt$layout$name == "panel"] <- "off"
grid.draw(gt)
ggsave("../plots/paper/theory_only_fig3_B.png", plot=gt, width=3, height=3)
```
# supplemental alignment time figure
    ...
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (aligned in alignments) {
            for (run in 0:(nruns-1)) { 
              this_d = read.csv(sprintf("../sg_%il_results/n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_aligned_%s_run_%i_SVD_track.csv", depth, n2b, noise_var, output_size, svm, aligned, run), header=T)
        
              this_d = this_d %>% 
                mutate(noise_var=noise_var, output_size=output_size, svm=svm, run=run, teacher_rank=n2b, depth=depth, student_rank="Full rank", type="Empirical")
              d = bind_rows(d, this_d)
            }
          }
        }
...
d2 = d %>%
  mutate(output_size=factor(output_size),
         ...
         type=factor(type),
         student_rank=factor(student_rank, levels=c("Full rank", "50% of full", "4% of full", "Teacher rank (1% of full)"), labels=c("Full rank", "50% of full", "4% of full", "1% of full"))) %>%
  gather(observation, value, matches("hat")) %>% #gather all SVD obsvs
  separate(observation, c("obs_type", "mode"), sep=c(-1)) %>%
  mutate(obs_type=factor(obs_type, levels=c("UUhat", "VVhat", "NormShat"), labels=c("U", "V", "s")),
         value=abs(value)) # plots are less confusing if alignments are positive, since signs are arbitrary
```

```{r}
make_alignment_time_plot = function(d2, file_prefix, let, x_breaks=c(0., 5., 10.), legend=T) {
  p = ggplot(data=d2, aes(x=epoch/1000, y=value, color=obs_type)) +
    geom_point() +
    scale_color_brewer(palette="Dark2",
                       breaks = c("U", "V", "s"),
                       labels = c(TeX("\\hat{u}}"), TeX("{\\bf v}\\cdot{\\bf\\hat{v}}"), TeX("s/\\hat{s}"))) + # TODO: Don't know why this doesn't work, debug given time
    scale_x_continuous(breaks=x_breaks) +
    labs(x=TeX("$t/\\tau$"),
         y="Alignment") +
    scale_y_continuous(breaks=c(0., 1.)) +
    annotation_custom(textGrob(label = let, hjust = 2, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        )
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, let, ".png", sep=""), plot=gt, width=ifelse(legend, 4, 3), height=3)
}
```

```{r}
make_alignment_time_plot(d2 %>% filter(depth == 1,
                                       teacher_rank == 1),
                         file_prefix="../plots/paper/supp_alignment_time_",
                         let="A")
...
```{r}
make_alignment_time_plot(d2 %>% filter(depth == 3,
                                       teacher_rank == 1),
                         file_prefix="../plots/paper/supp_alignment_time_",
                         let="E",
                         x_breaks=c(0., 75., 150.))
    ...
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (aligned in alignments) {
            for (run in 0:(nruns-1)) { 
              filename = sprintf("../sg_nl_results_orth_leaky/depth_%i_n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_run_%i_.csv", depth, n2b, noise_var, output_size, svm, run)
              if (!file.exists(filename)) {
                next
              }
              this_d = read.csv(filename, header=T)
              ...
              this_d = this_d %>% 
                mutate(noise_var=noise_var, output_size=output_size, svm=svm, aligned=aligned=="True", run=run, depth=depth-1, teacher_rank=n2b, student_rank="Full rank", type="Empirical")
              d = bind_rows(d, this_d)
            }
          }
        }
...
```{r}
avg_d = d %>%
  group_by(depth, noise_var, output_size, svm, epoch, teacher_rank, type) %>%
  summarize(loss=mean(loss), log_tr_loss_sd=sd(log_train_loss), log_tr_loss=mean(log_train_loss), log_loss_sd=sd(log_loss), log_loss=mean(log_loss)) %>%
  mutate(log_loss_95_low=log_loss-1.96*log_loss_sd/sqrt(n()), log_loss_95_high=log_loss+1.96*log_loss_sd/sqrt(n()),
         log_tr_loss_95_low=log_tr_loss-1.96*log_tr_loss_sd/sqrt(n()), log_tr_loss_95_high=log_tr_loss+1.96*log_tr_loss_sd/sqrt(n())) %>%
  ungroup() %>%
...
```{r}
make_nonlinear_fig_plots = function(avg_d, file_prefix, lets, x_breaks=c(0., 5., 10.), tau=1000, legend=F) {
  # train 
  p = ggplot(avg_d %>% filter(epoch %% 50 == 0), aes(x=epoch/tau, y=log_tr_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm))) +
    geom_line() +
    geom_point(size=2) +
    scale_x_continuous(breaks=x_breaks) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 30), breaks=c(1., 30)) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{train}$")) +
    annotation_custom(textGrob(label = lets[1], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
  ...
  if (legend) {
    p = p + guides(color=guide_colorbar(title="SNR", order=1))
    
  } else {
    p = p + guides(color=F, linetype=F, shape=F)
  }
  
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[1], ".png", sep=""), plot=gt, width=ifelse(legend,4,3), height=3)
  
  # test
  p = ggplot(avg_d %>% filter(epoch %% 50 == 0), aes(x=epoch/tau, y=log_loss, color=as.numeric(levels(svm)[svm]), group=interaction(new_type, svm))) +
    geom_line() +
    geom_point(size=2) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 30), breaks=c(1., 30)) +
    scale_x_continuous(breaks=x_breaks) +
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[2], ".png", sep=""), plot=gt, width=ifelse(legend,4,3), height=3)

}
```

```{r}
make_nonlinear_fig_plots(avg_d %>% filter(depth==1,
                                          epoch <= 25000,
                                          teacher_rank==1),
                         file_prefix="../plots/paper/nl_fig_redux_",
                         x_breaks=c(0., 25.),
                         lets=c("A", "E"))
```
...
```{r}
make_nonlinear_fig_plots(avg_d %>% filter(depth==3,
                                          teacher_rank==1),
                         file_prefix="../plots/paper/nl_fig_redux_",
                         lets=c("C", "G"),
                         tau=10000,
  ...
  for (n2b in N_2_bars) {
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (run in 0:(nruns-1)) { 
            filename = sprintf("../sg_nl_new_results/depth_%i_n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_run_%i_.csv", depth, n2b, noise_var, output_size, svm, run)
            if (!file.exists(filename)) {
              next
            }
            this_d = read.csv(filename, header=T)
            ...
            this_d = this_d %>% 
              mutate(noise_var=noise_var, output_size=output_size, svm=svm, run=run, depth=depth-1, teacher_rank=n2b, student_rank="Full rank", type="Empirical")
            d = bind_rows(d, this_d)
          }
        }
      }
...
avg_d = d %>%
  ...
  mutate(new_type = ifelse(type == "Theory", "Theory",
                           "Empirical random")) %>%
  mutate(new_type = factor(new_type, levels=c("Theory", "Empirical random")),
         aligned = F)
```


```{r}
make_nonlinear_fig_plots(avg_d %>% filter(epoch <= 400000),
                         file_prefix="../plots/other/nl_fig_relu_",
                         lets=c("D", "H"),
                         x_breaks=c(0., 20., 40.),
                         tau=10000,
                         legend=T)
...
#  Randomizing slows training

```{r}
alignments=c("False")
nruns = 1
randomizeds = c("True", "False")
N_2_bars = c(3)
noise_vars = c(1.)
output_sizes= c(50)
svms = c(10.)
d = data.frame()
spectra_d = data.frame()
for (rand in randomizeds) {
  for (n2b in N_2_bars) {
    for (noise_var in noise_vars) {
      for (output_size in output_sizes) {
        for (svm in svms) {
          for (aligned in alignments) {
            for (run in 0:(nruns-1)) { 
              filename = sprintf("../randomizing_data_results/rand_%s_n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_aligned_%s_run_%i.csv", rand, n2b, noise_var, output_size, svm, aligned,run)

              this_d = read.csv(filename, header=T)
        
              this_d = this_d %>% 
                mutate(noise_var=noise_var, output_size=output_size, svm=svm, aligned=aligned=="True", run=run, randomized=rand, teacher_rank=n2b, student_rank="Full rank", type="Empirical")
              d = bind_rows(d, this_d)
              
              filename = sprintf("../randomizing_data_results/rand_%s_n2b_%i_noise_var_%.2f_output_size_%i_svm_%f_aligned_%s_run_%i_noisy_y_data_spectrum.csv", rand, n2b, noise_var, output_size, svm, aligned,run)

              this_d = read.csv(filename, header=F)
              this_d = this_d %>% 
                mutate(mode=row.names(this_d), singular_value=V1, noise_var=noise_var, output_size=output_size, svm=svm, aligned=aligned=="True", run=run, randomized=rand, teacher_rank=n2b, student_rank="Full rank", type="Empirical")
              spectra_d = bind_rows(spectra_d, this_d)
            }
          }
        }
      }
...
```{r}
avg_d = d %>%
  group_by(randomized, noise_var, output_size, svm, epoch, teacher_rank, type) %>%
  summarize(loss=mean(loss), tr_loss_sd=sd(train_loss), tr_loss=mean(train_loss), loss_sd=sd(loss), loss=mean(loss)) %>%
  mutate(loss_95_low=loss-1.96*loss_sd/sqrt(n()), loss_95_high=loss+1.96*loss_sd/sqrt(n()),
         tr_loss_95_low=tr_loss-1.96*tr_loss_sd/sqrt(n()), tr_loss_95_high=tr_loss+1.96*tr_loss_sd/sqrt(n())) %>%
  ungroup() %>%
  mutate(new_type = ifelse(type == "Theory", "Theory",
                            ifelse(aligned == "TRUE", "Empirical aligned", "Empirical random"))) %>%
  mutate(new_type = factor(new_type, levels=c("Theory", "Empirical aligned", "Empirical random")),
         aligned=factor(aligned, levels=c("TRUE", "FALSE"), labels=c("TA", "Random")),
         randomized=factor(randomized, levels=c("False", "True"), labels=c( "Original", "Randomized")))
```
```{r}
spectra_d = spectra_d %>%
  mutate(randomized=factor(randomized, levels=c("False", "True"), labels=c( "Original", "Randomized")))
```

```{r}
make_randomizing_fig_plots = function(avg_d, spectra_d, file_prefix, lets, x_breaks=c(0., 2.)) {
    # spectra
    theory_colors = c("#B3DE69", "#FDB462")
    p = ggplot(spectra_d, aes(x=singular_value, fill=randomized)) +
    geom_histogram(position="dodge", binwidth=0.5) +
    scale_fill_brewer(palette="Set1", drop=F) +
    scale_color_manual(breaks=c("Signal values", "Noise endpoints"), values=theory_colors) +
    labs(x=TeX("$s$"),
         y=TeX("Count")) +
    annotation_custom(textGrob(label = lets[1], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) +
    facet_grid(randomized ~ .) +
    guides(fill=guide_legend(title="Data"), color=guide_legend(title="Theory")) +
    theme(strip.text=element_blank(), strip.background = element_blank())
  

  this_svm = spectra_d$svm[1]
  # why yes, this is a manually unrolled for loop to get around stupid variable weirdness with ggplot
  s = 1.
  this_s = s * this_svm
  inflated1 = sqrt((1+this_s^2)*(0.5+this_s^2))/this_s
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Original"), aes(xintercept=inflated1, color="Signal values"), linetype="dashed", size=0.66)
  s=2.
  this_s = s * this_svm
  inflated2 = sqrt((1+this_s^2)*(0.5+this_s^2))/this_s
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Original"), aes(xintercept=inflated2, color="Signal values"), linetype="dashed", size=0.66)
  s=3.
  this_s = s * this_svm
  inflated3 = sqrt((1+this_s^2)*(0.5+this_s^2))/this_s
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Original"), aes(xintercept=inflated3, color="Signal values"), linetype="dashed", size=0.66)
  
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Original"), aes(xintercept=1-sqrt(0.5), color="Noise endpoints"), linetype="dashed", size=0.66)
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Original"), aes(xintercept=1+sqrt(0.5), color="Noise endpoints"), linetype="dashed", size=0.66)
  
  
  sigma_r = sqrt(sum((c(1., 2., 3.)*this_svm)^2)/50 + 1)
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Randomized"), aes(xintercept=sigma_r*(1-sqrt(0.5)), color="Noise endpoints"), linetype="dashed", size=0.66)
  p = p +
    geom_vline(data=spectra_d %>% filter(randomized=="Randomized"), aes(xintercept=sigma_r*(1+sqrt(0.5)), color="Noise endpoints"), linetype="dashed", size=0.66)
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel-1-1"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[1], ".png", sep=""), plot=gt, width=4.5, height=3)
  

  # train 
  p = ggplot(avg_d %>% filter(epoch %% 10 == 0), aes(x=epoch/1000, y=tr_loss, color=randomized, group=interaction(randomized, svm))) +
    geom_line() +
    geom_point(size=2) +
    scale_x_continuous(breaks=x_breaks, limits=c(x_breaks[1], x_breaks[2])) +
    scale_color_brewer(palette="Set1", drop=F) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\epsilon_{train}$")) +
    annotation_custom(textGrob(label = lets[2], hjust = 3, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")),
        ) +
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[2], ".png", sep=""), plot=gt, width=4.5, height=3)
}
```

```{r}
make_randomizing_fig_plots(avg_d,
                           spectra_d,
                           file_prefix = "../plots/paper/randomized_fig_",
                           lets=c("A", "B"))
```
...
```{r}
nruns = 10
ps = c(20, 100, 200, 40, 60, 80, 120, 140, 160, 180)
noise_vars = c(1.)
output_sizes= c(100)
svms = c(2., 3., 4.)
alignments = c("True")
          ...
          for (aligned in alignments) {
            for (input_type in input_types) {
              if (input_type == "orthogonal") {
                this_dir = "changing_p_results"
              } else {
                this_dir = "changing_p_results_gaussian"
              }
              for (run in 0:(nruns-1)) { 
                filename = sprintf("../%s/noise_var_%.2f_output_size_%i_input_size_100_num_examples_%i_input_type_%s_svm_%f_aligned_%s_run_%i.csv", this_dir, noise_var, output_size, p, input_type, svm, aligned, run)
                if (!file.exists(filename)) {
                ...
                this_d = this_d %>% 
                  mutate(noise_var=noise_var, output_size=output_size, svm=svm, aligned=aligned=="True", input_type=input_type, run=run, p=p, depth=1, teacher_rank=1, student_rank="Full rank", type="Empirical")
                d = bind_rows(d, this_d)
              }
            }
          }
          theory_filename = sprintf("../changing_p_results/n2b_1_noise_var_%.2f_p_%i_svm_%f_theory_track.csv", noise_var,  p, svm)
          if(file.exists(theory_filename)) { # min_d only theory is omitted
            this_d = read.csv(theory_filename, header=T)
          
            this_d = this_d %>% 
              rename(loss=generalization_error) %>%
              mutate(noise_var=noise_var, output_size=NA,  svm=svm, aligned=aligned=="True", p=p, depth=1, teacher_rank=1, input_type=NA, run=NA, student_rank="Full rank", type="Theory")
            d = bind_rows(d, this_d)
          }
        }
      }
...
```{r}
d = d %>%
  mutate(rt_D_x_snr = sqrt(p/100)*svm,
         output_size=factor(output_size),
         train_loss=ifelse(type=="Theory", train_error, train_loss),
         noise_var=factor(noise_var),
         p=factor(p),
         svm=factor(svm),
...
```{r}
avg_d = d %>%
  filter(svm == 3., p %in% c(20, 100, 200), input_type=="orthogonal" | type == "Theory") %>%
  group_by(depth, p, noise_var, output_size, svm, aligned, epoch, teacher_rank,student_rank, type) %>%
  summarize(loss=mean(loss), log_tr_loss_sd=sd(log_train_loss), log_tr_loss=mean(log_train_loss), log_loss_sd=sd(log_loss), log_loss=mean(log_loss)) %>%
  mutate(log_loss_95_low=log_loss-1.96*log_loss_sd/sqrt(n()), log_loss_95_high=log_loss+1.96*log_loss_sd/sqrt(n()),
         log_tr_loss_95_low=log_tr_loss-1.96*log_tr_loss_sd/sqrt(n()), log_tr_loss_95_high=log_tr_loss+1.96*log_tr_loss_sd/sqrt(n())) %>%
  ungroup() %>%
...
```{r}
min_d = d %>%
  group_by(svm, rt_D_x_snr, p, depth, aligned, noise_var, type, teacher_rank, input_type, student_rank, run) %>%
  summarize(min_Eg = min(log_loss), min_t = which.min(log_loss)) %>%
  ungroup() %>%
  group_by(svm, p, rt_D_x_snr, depth, aligned, noise_var, teacher_rank, input_type, student_rank, type) %>%
  summarize(avg_min_Eg = mean(min_Eg), sd_min_Eg = sd(min_Eg),
            avg_min_t = mean(min_t), sd_min_t = sd(min_t),
            se_min_Eg = sd_min_Eg/sqrt(n()), ci_Eg_up= avg_min_Eg + 1.96*se_min_Eg, ci_Eg_lo= avg_min_Eg - 1.96*se_min_Eg,
            se_min_t = sd_min_t/sqrt(n()), ci_t_up= avg_min_t + 1.96*se_min_t, ci_t_lo= avg_min_t - 1.96*se_min_t) %>%
...
wide_min_d = min_d %>% 
  ...
  select(-type, -se_min_Eg, -se_min_t) %>%
  gather(metric, measurement, avg_min_Eg, sd_min_Eg, avg_min_t, sd_min_t, ci_t_up, ci_t_lo, ci_Eg_up, ci_Eg_lo) %>%
  unite(itm, input_type, metric) %>%
  spread(itm, measurement) 
```

```{r}
make_changing_p_plots = function(avg_d, min_d, wide_min_d, file_prefix, lets, x_breaks=c(0., 5., 10.)) {
  # train
  p = ggplot(avg_d %>% filter(type=="Theory", svm==3.), aes(x=epoch/(1000), y=log_tr_loss, color=as.numeric(levels(p)[p])/100, group=interaction(svm, p))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), svm==3., epoch %% 500 == 0), aes(shape=aligned), size=3) +
    scale_x_continuous(breaks=x_breaks) +
    scale_color_gradient(low="#008800", high="#FFD700", limits=c(0, 2), breaks=c(0.2, 1., 2.)) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{train}$")) +
    guides(color=F, linetype=F, shape=F)+
    annotation_custom(textGrob(label = lets[1], hjust = 2.5, gp = gpar(cex = 2),
  ...
  # test
  p = ggplot(avg_d %>% filter(type=="Theory"), aes(x=epoch/(1000), y=log_loss, color=as.numeric(levels(p)[p])/100, group=interaction(svm, p))) +
   geom_line(aes(linetype=factor("Theory")), size=1.5) +
    geom_point(data=avg_d %>% filter(grepl("Empirical", type), epoch %% 500 == 0), aes(shape=aligned), size=3) +
    scale_color_gradient(low="#008800", high="#FFD700", limits=c(0, 2), breaks=c(0.2, 1., 2.)) +
    scale_x_continuous(breaks=x_breaks) +
    labs(x=TeX("$t/\\tau$"),
         y=TeX("$\\ln \\, \\epsilon_{test}$")) +
    guides(color=guide_colorbar(title=TeX("$P/N_1$"), order=3), linetype=guide_legend(title="", order=1), shape=guide_legend(title="Empirical", order=2))+
    annotation_custom(textGrob(label = lets[2], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.95, "npc")))  +
    theme(legend.margin = margin(-0.1,0,-0.1,0, unit="cm"))
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[2], ".png", sep=""), plot=gt, width=4, height=3)
  
  p = ggplot(data=min_d %>% filter(type=="Empirical", input_type=="orthogonal"), aes(x=sqrt(as.numeric(levels(p)[p])/100), y=avg_min_Eg, color=as.numeric(levels(svm)[svm]))) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(2, 4), breaks=c(2, 3, 4)) +
    labs(x=TeX("\\sqrt{P/N_1} "),
         y=TeX("$\\min \\, \\ln \\,\\epsilon_{test}$ (TA)")) +
    geom_line(data=min_d %>% filter(type=="Theory"), aes(group=svm), size=1.5) +
    geom_errorbar(aes(ymin=ci_Eg_lo, ymax=ci_Eg_up), width=0.05) +
    geom_point(size=3) +
    guides(color=F, shape=F)  +
    annotation_custom(textGrob(label = lets[3], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.97, "npc")),
  ...
  p = ggplot(data=min_d %>% filter(type=="Empirical", input_type=="orthogonal"), aes(x=rt_D_x_snr, y=avg_min_Eg, color=as.numeric(levels(svm)[svm]))) +
    scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(2, 4), breaks=c(2, 3, 4)) +
    labs(x=TeX("SNR\\sqrt{P/N_1} "),
         y=TeX("$\\min \\, \\ln \\,\\epsilon_{test}$ (TA)")) +
    geom_line(data=min_d %>% filter(type=="Theory"), aes(group=svm), size=1.5) +
    geom_errorbar(aes(ymin=ci_Eg_lo, ymax=ci_Eg_up), width=0.05) +
    geom_point(size=3) +
    guides(color=guide_colorbar(title="SNR"), shape=F)  +
    annotation_custom(textGrob(label = lets[4], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.97, "npc")),
        ) 
  ...
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[4], ".png", sep=""), plot=gt, width=4, height=3)
  
  p = ggplot(data=wide_min_d, aes(x=orthogonal_avg_min_Eg, y=gaussian_avg_min_Eg, color=as.numeric(levels(p)[p])/100, shape=svm)) +
    geom_abline(intercept=0, slope=1, alpha=0.5, linetype=3) +
    scale_color_gradient(low="#008800", high="#FFD700", limits=c(0, 2), breaks=c(0.2, 1., 2.)) +
    labs(x=TeX("$\\min \\, \\ln \\,\\epsilon_{test}$ (Orth.)"),
         y=TeX("$\\min \\, \\ln \\,\\epsilon_{test}$ (Gauss.)    ")) +
    geom_errorbar(aes(ymin=gaussian_ci_Eg_lo, ymax=gaussian_ci_Eg_up), width=0.05) +
    geom_errorbarh(aes(xmin=orthogonal_ci_Eg_lo, xmax=orthogonal_ci_Eg_up), height=0.05) +
    scale_x_continuous(limits=c(-3., 0.)) +
    scale_y_continuous(limits=c(-3., 0.)) +
    geom_point(size=2) +
    guides(color=guide_colorbar(title=TeX("$P/N_1$"), barheight=unit(2.5, "cm"), order=1),
           shape=guide_legend(title="SNR"))  +
    annotation_custom(textGrob(label = lets[5], hjust = 2.5, gp = gpar(cex = 2),
        x=unit(0, "npc"), y=unit(0.97, "npc"))) +
    theme(legend.margin = margin(0,0,-0.2,0, unit="cm"))
  
  gt <- ggplot_gtable(ggplot_build(p))
  gt$layout$clip[gt$layout$name == "panel"] <- "off"
  grid.draw(gt)
  ggsave(paste(file_prefix, lets[5], ".png", sep=""), plot=gt, width=4, height=3)
...
```{r}
make_changing_p_plots(avg_d, min_d, wide_min_d,
                      file_prefix="../plots/paper/changing_p_",
#                      lets=c("A", "B", "C", "D", "E"))
                      lets=c("blah", "A", "B", "C", "D", "E"))
```
