...
```{r}
ggplot(data=avg_d %>% filter(epoch %% 20==0, svm < 100, !aligned, mean_loss < 5.), aes(x=epoch, y= mean_loss, color=svm)) + 
  geom_point() +
...
```{r}
ggsave("../plots/paper/fewer_inputs.png")
```


```{r}
ggplot(data=avg_d %>% filter(epoch %% 10==0, svm < 100, !aligned), aes(x=epoch, y= mean_loss, color=svm)) + 
  geom_point() +
  scale_color_gradient(low="#0000FF", high="#FF0000", limits=c(0, 10), breaks=c(0.84, 10), labels=c( parse(text="sqrt(1/2, 4)"), "10      ")) +
  facet_wrap(~num_inputs) +
  labs(y="Log testing loss") +
  xlim(0, 500) +
  ylim(-2.5,5)
```

```{r}
ggsave("../plots/paper/fewer_inputs_zoomed_in.png")
```
